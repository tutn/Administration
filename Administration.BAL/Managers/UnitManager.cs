...
namespace Administration.BAL.Managers
{
    public class UnitManager : IUnitManager
    {
        ...
        public PagingResult Search(UNIT_Params model)
        {
        ...
        public Result Add(SYS_UNITS model)
        {
            ...
            try
            {
                ...
                using (IUnitOfWork unitOfWork = new UnitOfWork())
                {
                    var data = unitOfWork.UnitRepository.FirstOrDefault(x => x.UNIT_ID == model.UNIT_ID);
                    if (data != null)
                    {
        ...
        public Result Delete(SYS_UNITS model)
        {
            ...
            try
            {
                ...
                using (IUnitOfWork unitOfWork = new UnitOfWork())
                {
                    var data = unitOfWork.UnitRepository.GetById(model.UNIT_ID);
                    if (data == null)
                    {
                        result.Code = (short)HttpStatusCode.NotFound;
                        result.Message = string.Format("The Unit did not find. Please check it again!");
            ...
            try
            {
                using (IUnitOfWork unitOfWork = new UnitOfWork())
                {
                    ...
                    if (UnitId != null && UnitId != 0)
                    {
                        ...
                        var subIds = subdata.Select(s => s.UNIT_ID).ToList();
                        subIds.Add(UnitId.Value);
                        var newdata = data.Where(x => !subIds.Contains(x.UNIT_ID)).ToList();
        ...
        public Result GetUnitNodes()
        {
            ...
            try
            {
                using (IUnitOfWork unitOfWork = new UnitOfWork())
                {
                    ...
                    var dataList = new List<UNIT_NODE>();
        ...
        private List<SYS_UNITS> GetChildren(IQueryable<SYS_UNITS> dataList, int? parentId, string prefixc, string parentprefix, List<int> disableIds, bool isSearch)
        {
            ...
            var objs = dataList.Where(f => f.PARENT_ID == parentId).OrderBy(o => o.UNIT_ID);
            ...
            foreach (var item in objs)
            {
                ...
                {
                    if (disableIds.Contains(item.UNIT_ID))
                    {
                ...
                dataLst.Add(item);
                var subdata = GetChildren(dataList, item.UNIT_ID, prefixc, prefixcharactor, disableIds, isSearch);
        ...
        private List<UNIT_NODE> GetNodes(IQueryable<TBL_SYS_UNITS> dataList, int? parentId)
        {
            var dataLst = new List<UNIT_NODE>();
            var objs = dataList.Where(f => f.PARENT_ID == parentId).OrderBy(o => o.UNIT_ID);
            UNIT_NODE node;
            foreach (var item in objs)
            {
                node = new UNIT_NODE();
                node.ID = item.UNIT_ID.ToString();
                node.NAME = item.NAME;
                node.SELECTED = false;
                node.children = GetNodes(dataList, item.UNIT_ID);
