...
namespace Administration.BAL.Managers
{
    public class UserManager : IUserManager
    {
        ...
        public PagingResult Search(USER_Params model)
        {
        ...
        public Result Add(SYS_USERS model)
        {
            ...
            try
            {
                ...
                using (IUnitOfWork unitOfWork = new UnitOfWork())
                {
                    var data = unitOfWork.UserRepository.FirstOrDefault(x => x.USER_ID == model.USER_ID);
                    if (data != null)
                    {
        ...
        public Result Delete(SYS_USERS model)
        {
            ...
            try
            {
                ...
                using (IUnitOfWork unitOfWork = new UnitOfWork())
                {
                    var data = unitOfWork.UserRepository.GetById(model.USER_ID);
                    if (data == null)
                    {
                        result.Code = (short)HttpStatusCode.NotFound;
                        result.Message = string.Format("The User did not find. Please check it again!");
